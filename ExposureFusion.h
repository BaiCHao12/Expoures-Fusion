...
public:
	QualityMeasures(Mat img, Mat gimg)
	{
		...
		this->WellExposedness = getWellExposednessMeasure(gimg);
...
public:
	ExposureFusion(char* seqPath)
	{
		...
		std::ifstream file(nfrmfname); // 打开文件
		if (!file.is_open()) {
		...
		std::string line;
		std::vector<std::string> vTxTContent;
		while (getline(file, line)) { // 逐行读取文件内容
			vTxTContent.push_back(line);
		}
		file.close(); // 关闭文件

		this->nframes = vTxTContent.size();
		

		for(int n=0; n<nframes; n++)
		{
			sprintf(seqfname, "%s\\%s", seqPath, vTxTContent[n].c_str());
			Mat inputimg = imread(seqfname, IMREAD_COLOR);	// flag 函版夸噶
			if (!inputimg.data)
			{
	...
	ExposureFusion(std::vector<std::string> vFileName)
	{
		this->nframes = vFileName.size();
		for (int n = 0; n < nframes; n++)
		{
			Mat inputimg = imread(vFileName[n], IMREAD_COLOR);	// flag 函版夸噶
			if (!inputimg.data)
			{
				cout << "fail to read image" << endl;
				exit(-1);
	...
	void RobustNormalization();
	
	bool SaveImageBMP(std::string filename)
	{
		if (resultimage.data) {
			if(filename==std::string(""))
				cv::imwrite("EF_Output.jpg", resultimage);
			else
